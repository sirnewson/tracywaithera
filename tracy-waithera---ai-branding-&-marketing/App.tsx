...
import { 
  Menu, X, Sparkles, ShieldCheck, Compass, Mail, Phone, MapPin, 
  Award, Heart, Bot, Mic, Wallet, BookOpen, FileText, Clock, Zap, ArrowRight, MessageSquare, Play
} from 'lucide-react';
import { STRINGS, SERVICES, VIDEOS, RATE_CARD, ARTICLES, SOCIAL_LINKS } from './constants';
    ...
    <div className="relative min-h-screen bg-white text-zinc-800 selection:bg-orange-500 selection:text-white font-sans">
      ...
      <div className={`splash-overlay ${!showSplash ? 'hide' : ''}`}>
        <div className="relative flex flex-col items-center px-6 text-center">
          <img src={LOGO_URL} alt="Tracy Waithera Logo" className="w-24 h-24 rounded-2xl mb-6 shadow-xl animate-bounce" />
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-zinc-900 z-10 uppercase">
            TRACY <span className="gradient-text italic">WAITHERA</span>
          </h1>
          <p className="mt-4 text-[9px] font-black uppercase tracking-[0.4em] text-zinc-400 z-10">Studio of AI Strategy</p>
        </div>
      </div>
      ...
      {/* Hero Section - Optimized Visibility & Headroom */}
        ...
        <div className="max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-12 lg:gap-24 items-center relative z-10">
          ...
          <div className="order-2 md:order-1 flex flex-col items-center md:items-start text-center md:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 w-full sm:w-auto">
              <button onClick={() => scrollTo('ai-tool')} className="w-full sm:w-auto bg-orange-600 hover:bg-orange-700 text-white font-black px-12 py-5 rounded-2xl flex items-center justify-center gap-3 transition-all hover:scale-105 shadow-xl shadow-orange-500/20 uppercase tracking-widest text-[10px]">
                {t.hero.ctaAudit} <Zap size={18} />
              </button>
          ...
          {/* Hero Portrait - Optimized for Visibility and Headroom */}
      ...
      {/* AI Tool Section */}
      <section id="ai-tool" className="py-32 px-6 bg-grid">
        <div className="max-w-7xl mx-auto">
          <StrategyTool lang={lang} t={t} />
        </div>
      </section>
      ...
      <section id="agency" className="py-32 px-6 bg-zinc-950 text-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center relative z-10">
           <div>
              ...
              <button onClick={() => sendWhatsApp()} className="bg-white text-zinc-900 font-black px-10 py-5 rounded-2xl hover:bg-orange-500 hover:text-white transition-all uppercase tracking-widest text-[10px]">
                {t.trouve.partner}
              </button>
           </div>
      ...
      <section id="insights" className="py-32 px-6 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
            <h2 className="text-4xl md:text-6xl font-black text-zinc-900 uppercase tracking-tighter">{t.articles.title}</h2>
            <button onClick={() => scrollTo('contact')} className="text-[11px] font-black text-zinc-400 hover:text-orange-600 uppercase tracking-widest transition-colors flex items-center gap-2">Explore Hub <ArrowRight size={14} /></button>
          </div>

          <div className="grid md:grid-cols-3 gap-10">
            {ARTICLES.map((article, i) => (
              <div key={i} onClick={() => handleArticleClick(article)} className="group cursor-pointer flex flex-col rounded-[2.5rem] bg-zinc-50 border border-zinc-100 hover:bg-white hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden">
                <div className="aspect-[16/10] overflow-hidden">
                   <img src={article.image} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                </div>
                <div className="p-10 flex flex-col flex-1">
                  <div className="mb-6 flex justify-between items-center text-[9px] font-black text-zinc-400 uppercase tracking-widest">
                     <span>{article.readingTime}</span>
                     <span>{article.date}</span>
                  </div>
                  <h3 className="text-2xl font-black text-zinc-900 mb-4 uppercase tracking-tight group-hover:text-orange-600 transition-colors leading-tight">{article.title}</h3>
                  <p className="text-zinc-500 text-sm font-normal mb-8 leading-relaxed line-clamp-3">{article.excerpt}</p>
                  <div className="mt-auto flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-zinc-900">
      ...
      <section id="videos" className="py-32 px-6 bg-zinc-900 overflow-hidden">
        ...
        <div className="netflix-scroll flex gap-6 overflow-x-auto px-10 pb-10 snap-x">
          {VIDEOS.map((video, i) => (
            <div key={i} className="flex-none w-[300px] md:w-[420px] snap-center group cursor-pointer">
